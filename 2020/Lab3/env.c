...
env_setup_vm(struct Env *e)
{
     ...
     */
        ...
        //set e->env_pgdir and e->env_cr3 accordingly.
        e->env_pgdir = pgdir;
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
   ...
   /* for(; bin_size > BY2PG && i < bin_size-BY2PG; i += BY2PG) {
        ...
        //for (i = 0; bin_size > BY2PG && i < bin_size-BY2PG; i += BY2PG) {
        /* Hint: You should alloc a new page. */
        /*if(page_alloc(&p)<0){
                printf("Error Load mapper!\n");
                return -E_NO_MEM;
        }
        //p->pp_ref++;
        if(i == 0){
